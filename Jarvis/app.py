...
if os.getenv("RENDER") is None:
    ...
    # Define dummy functions/classes to prevent NameErrors on Render
    def playsound(*args, **kwargs):
        print("[playsound skipped in Render]")
...
def speak(text: str) -> None:
    ...
    try:
        ...
        # Use NamedTemporaryFile but close/remove safely for playsound
        ...
        try:
            ...
            if os.getenv("RENDER") is None:
                ...
                print("[Audio playback skipped on Render]")
        finally:
            try:
                os.remove(tmp_path)
            except Exception:
...
def ask():
    ...
    # Speak the reply aloud (threaded)
...
def get_history(session_id):
    """Retrieve stored chat history for a session and serialize datetimes to ISO strings."""
    try:
        if conversations is None:
            return jsonify({"error": "Database unavailable"}), 500
        ...
        if convo:
            ...
            if "created_at" in convo and isinstance(convo["created_at"], datetime):
...
def start_tray():
    ...
    if os.getenv("RENDER") is None:
        ...
        # On Render, just run Flask
        run_flask()

@app.route("/clear_history/<session_id>", methods=["DELETE"])
